	...
	if (file.lineas[contadorPasoAPaso][1].trim().includes('//')) {
		...
		for (etiquetas of file.etiquetas) {
			if (etiquetas.nombre == file.lineas[contadorPasoAPaso][2].trim()) {
				if (
					...
					etiquetas.valor < file.lineas[contadorPasoAPaso - 1]
				) {
		...
		if (Number(acumulador) > 0) {
			for (e of file.etiquetas) {
				if (
					file.lineas[contadorPasoAPaso][2].trim().toLowerCase() ==
			...
			for (e of file.etiquetas) {
				if (
					file.lineas[contadorPasoAPaso][3].trim().toLowerCase() ==
						e.nombre.trim().toLowerCase() &&
					e.sobrepasa === false
				) {
					contadorPasoAPaso = Number(e.valor) - 2;
		...
		for (potencia of file.variables) {
			if (
				file.lineas[contadorPasoAPaso][2] == potencia.nombre &&
		...
		nextFile++;
		contadorPasoAPaso = -1;
	} else if (file.lineas[contadorPasoAPaso][1].toLowerCase() == 'raiz') {
