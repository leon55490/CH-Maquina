...
function roundRobin(filesCH, quantum) {
	...
	for (let file = 0; file < filesCH.length; file++) {
		...
		for (
			let instruccion = filesCH[file].initialRr;
			instruccion < filesCH[file].endingRr;
			instruccion++
		) {
			...
			if (filesCH[file].lineas[instruccion][1].toLowerCase() == 'lea') {
				for (let variable = 0; variable < filesCH[file].variables.length; variable++) {
					...
					if (
						...
						filesCH[file].variables[variable].nombre
					) {
						let newValue = prompt(
							`Ingrese el VALOR de la variable ${filesCH[file].variables[variable].nombre}`
						);
				...
				for (let variable = 0; variable < filesCH[file].variables.length; variable++) {
					if (
						filesCH[file].lineas[instruccion][2].trim() ===
						filesCH[file].variables[variable].nombre
					) {
				...
				for (etiquetas of filesCH[file].etiquetas) {
					if (etiquetas.nombre == filesCH[file].lineas[instruccion][2].trim()) {
						if (
							...
							etiquetas.valor < filesCH[file].lineas[instruccion - 1]
						) {
							instruccion = Number(etiquetas.valor) - 2;
						} else {
							alert(
								`La etiqueta ${etiquetas.nombre} con un valor de ${etiquetas.valor}, sobrepasa la longitud del archivo que es= ${filesCH[file].lineas.length}`
							);
							location.reload();
						}
					}
				...
				if (Number(acumulador) > 0) {
					for (e of filesCH[file].etiquetas) {
						if (
							filesCH[file].lineas[instruccion][2].trim().toLowerCase() ==
					...
					for (e of filesCH[file].etiquetas) {
						if (
							filesCH[file].lineas[instruccion][3].trim().toLowerCase() ==
								e.nombre.trim().toLowerCase() &&
							e.sobrepasa === false
						) {
							console.log('Entró a la recursión');
				...
				for (potencia of filesCH[file].variables) {
					if (
						filesCH[file].lineas[instruccion][2] == potencia.nombre &&
				...
				// debugger;
			} else if (filesCH[file].lineas[instruccion][1].toLowerCase() == 'raiz') {
