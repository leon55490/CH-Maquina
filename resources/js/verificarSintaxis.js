	...
	for (let instruccion = 0; instruccion < lista.length; instruccion++) {
		...
		if (lista[instruccion][0].toLowerCase() == 'nueva') {
					...
					if (lista[instruccion].length > 3) {
						...
						for (let i = 0; i < lista[instruccion].length; i++) {
							if (!verificList.includes(Number(num))) {
								errores.push(
									'Error de sintaxis, el tipo de dato no es un entero: ' + linea
								);
								break;
							}
						}
					...
					if (lista[instruccion].length > 3) {
						...
						for (let i = 0; i < lista[instruccion].length; i++) {
							if (!verificList.includes(num)) {
								errores.push(
									'Error de sintaxis, el tipo de dato no es un Real/Decimal: ' + linea
								);
								break;
							}
						}
				...
				case 'L':
					if (lista[instruccion].length > 3) {
						if (!(lista[instruccion][3] == '0') && !(lista[instruccion][3] == '1')) {
							errores.push(
								'Error de sintaxis, el tipo de dato no es un Boolean: ' + linea
							);
							break;
						}
					} else {
			...
			if (!nombreVariables.includes(lista[instruccion][1])) {
				errores.push(
					'La variable ' + lista[instruccion][1] + ' no ha sido creada :' + linea
				);
			}
			...
			if (!nombreVariables.includes(lista[instruccion][1])) {
				...
				if (!nombreVariables.includes(lista[instruccion][2])) {
					errores.push(
						'La variable ' + lista[instruccion][2] + ' no ha sido creada: ' + linea
					);
				} else {
					if (!nombreVariables.includes(lista[instruccion][3])) {
						errores.push(
							'La variable ' + lista[instruccion][3] + ' no ha sido creada: ' + linea
						);
					}
				}
			}
			...
			if (
				!nombreVariables.includes(lista[instruccion][1]) &&
				!(lista[instruccion][1] == 'acumulador')
			) {
				errores.push(
					'La variable ' + lista[instruccion][1] + ' no ha sido creada: ' + linea
				);
