...
function runSrtn(filesCH) {
	...
	for (let file = 0; file < filesCH.length; file++) {
		...
		if (filesStateList.length === filesCH.length) {
			filesStateList = filesStateList.sort((a, b) => Number(a.cpu) - Number(b.cpu));
		}
		...
		for (
			let instruccion = filesStateList[file].initialRr;
			instruccion < filesStateList[file].endingRr;
			instruccion++
		) {
			...
			if (filesStateList[file].lineas[instruccion][1].toLowerCase() == 'lea') {
				for (
					let variable = 0;
					variable < filesStateList[file].variables.length;
					variable++
				) {
					// debugger;
					if (
						filesStateList[file].lineas[instruccion][2] ==
						filesStateList[file].variables[variable].nombre
					) {
						let newValue = prompt(
							`Ingrese el VALOR de la variable ${filesStateList[file].variables[variable].nombre}`
						);
				...
				for (
					...
					if (
						filesStateList[file].lineas[instruccion][2].trim() ===
						filesStateList[file].variables[variable].nombre
					) {
				...
				filesStateList[file].lineas[instruccion][1].toLowerCase() == 'almacene'
			) {
				...
				filesStateList[file].lineas[instruccion][1].trim().toLowerCase() === 'vaya'
			) {
				for (etiquetas of filesStateList[file].etiquetas) {
					if (etiquetas.nombre == filesStateList[file].lineas[instruccion][2].trim()) {
						if (
							...
							etiquetas.valor < filesStateList[file].lineas[instruccion - 1]
						) {
							instruccion = Number(etiquetas.valor) - 2;
						} else {
							alert(
								`La etiqueta ${etiquetas.nombre} con un valor de ${etiquetas.valor}, sobrepasa la longitud del archivo que es= ${filesStateList[file].lineas.length}`
							);
							location.reload();
						}
					}
				...
				filesStateList[file].lineas[instruccion][1].trim().toLowerCase() == 'vayasi'
			) {
				// debugger;
				if (Number(acumulador) > 0) {
					for (e of filesStateList[file].etiquetas) {
						if (
							filesStateList[file].lineas[instruccion][2].trim().toLowerCase() ==
					...
					for (e of filesStateList[file].etiquetas) {
						if (
							filesStateList[file].lineas[instruccion][3].trim().toLowerCase() ==
								e.nombre.trim().toLowerCase() &&
							e.sobrepasa === false
						) {
							console.log('Entró a la recursión');
				...
				filesStateList[file].lineas[instruccion][1].toLowerCase() == 'multiplique'
			) {
				...
				for (div of filesStateList[file].variables) {
					if (
						filesStateList[file].lineas[instruccion][2] == div.nombre &&
						div.valor != 0
					) {
				...
				filesStateList[file].lineas[instruccion][1].toLowerCase() == 'potencia'
			) {
				for (potencia of filesStateList[file].variables) {
					if (
						filesStateList[file].lineas[instruccion][2] == potencia.nombre &&
				...
				filesStateList[file].lineas[instruccion][1].toLowerCase() == 'concatene'
			) {
				...
				if (
					filesStateList[file].lineas[instruccion][2].toUpperCase().trim() == 'ACUMULADOR'
				) {
				...
				filesStateList[file].lineas[instruccion][1].toLowerCase() == 'extraiga'
			) {
				...
				// debugger;
			} else if (filesStateList[file].lineas[instruccion][1].toLowerCase() == 'raiz') {
